...
export const ChatRoom: React.FC = () => {
    ...
    useEffect(() => {
        ...
        socket.on("new-message", (data: { peerId: string; message: string }) => {
            const { peerId, message } = data;
            const msg: Message = { peerId, text: message };
        ...
        <div className="bg-gray-800 p-4 rounded-lg h-full w-96 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto mb-4">
                {messages.map((msg, index) => (
                    ...
                    >
                        <p className="text-sm ">{msg.peerId === user._id ? "You" : `User: ${msg.peerId}`}</p>
            ...
            <button
                onClick={sendMessage}
                className="text-sm font-bold text-white mt-2 bg-indigo-600 
                w-full h-10 rounded-lg shadow-md hover:bg-indigo-700 active:scale-95
                 active:shadow-lg focus:outline-none focus:ring-2
                focus:ring-indigo-300 transition-transform duration-200"
            >
                Send
            </button>
        </div>
